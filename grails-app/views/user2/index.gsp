            ...
            <table class="table table-striped table-centered table-nowrap mb-0 rounded">
                ...
                <g:each status="i" in="${userList}" var="user">
                    <tr class="${ (i % 2) == 0 ? 'a' : 'b'}">
                        <td>${i+1}</td>
                        <td class="fw-bold">
                            <a href="/user2/show/${user.id}">${user.username}</a>
                        </td>
                        <td>${user.nom}</td>
                        <td>${user.prenoms}</td>
                        <td>
                            <g:each in="${user.getAuthorities()}" var="role">
                                <span class="badge bg-primary">${role.authority}</span>
                            </g:each>
                        </td>
                        <td>${user.dateCreated}</td>
                        <td>
                            <a href="/user2/show/${user.id}" class="btn btn-secondary btn-sm">Consulter</a>
                        </td>
                    </tr>
                </g:each>
                </tbody>
